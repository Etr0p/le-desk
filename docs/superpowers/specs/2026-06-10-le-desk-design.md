...
| 9 | Produits structurés & pricing de structuration | Briques (zéro-coupon + options), pricing côté structureur : funding et prix du zéro-coupon, budget d'options, calcul de participation d'un capital garanti, coupon d'un reverse convertible (= prime du put vendu), mécanique et sensibilités d'un autocall, options à barrière, worst-of multi-sous-jacents (rôle de la corrélation et des dividendes), marge commerciale, Monte-Carlo appliqué au pricing, lecture et reconstitution d'un term sheet |
...
- **Phase 3 — La profondeur** : modules 5, 9, 10, 11, 12, 13 (crédit, structurés & pricing de structuration — dont un simulateur Monte-Carlo de pricing d'autocall interactif —, macro, crises, gestion/risques+ESG, brainteasers), calibrage de l'examen blanc façon Financial Game, glossaire/formulaire complets, polissage final (perf, accessibilité, responsive).
