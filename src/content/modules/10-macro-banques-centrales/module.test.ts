...
describe('module 10 — cibles de la spec (module culture, avec préférences utilisateur)', () => {
  const m = getModule('10-macro-banques-centrales')!;

  it('est complet', () => {
    expect(m.chapitres.length).toBe(8); // 7 chapitres + synthèse
    expect(m.exercices.length).toBeGreaterThanOrEqual(12);
    expect(m.problemes.length).toBeGreaterThanOrEqual(14);
    expect(m.problemes.reduce((a, p) => a + p.scenarios.length, 0)).toBeGreaterThanOrEqual(42);
    ...
    expect(m.formules.length).toBeGreaterThanOrEqual(12);
  });
