...
describe('flashcards module 3', () => {
  it('contient au moins 120 cartes', () => {
    expect(flashcards.length).toBeGreaterThanOrEqual(120);
  });

  it('a des ids uniques et bien formés couvrant m3-fc-001 à m3-fc-120', () => {
    const ids = flashcards.map(c => c.id);
    expect(new Set(ids).size).toBe(ids.length);
    for (const id of ids) {
      expect(id).toMatch(/^m3-fc-\d{3}$/);
    }
    const attendus = Array.from({ length: 120 }, (_, i) => `m3-fc-${String(i + 1).padStart(3, '0')}`);
  ...
  it('respecte les quotas par tag : 30 définitions, 16 formules, 14 ordres de grandeur, 24 pièges, 22 réflexes jury, 14 calculs mentaux', () => {
    const count = (tag: string) => flashcards.filter(c => c.tags.includes(tag)).length;
    expect(count('definition')).toBe(30);
    expect(count('formule')).toBe(16);
    expect(count('ordre-de-grandeur')).toBe(14);
    expect(count('piege')).toBe(24);
    expect(count('reflexe-jury')).toBe(22);
...
describe('glossaire du module 3', () => {
  const entreesM3 = glossaire.filter(e => e.moduleId === M3);

  it('contient au moins 32 entrées pour le module 3', () => {
    expect(entreesM3.length).toBeGreaterThanOrEqual(32);
  });

  it('chaque entrée m3 a un terme, un terme anglais (en) et des définitions FR/EN non vides', () => {
    for (const e of entreesM3) {
  ...
  it('les termes m3 sont uniques', () => {
    const termes = entreesM3.map(e => e.terme);
    expect(new Set(termes).size).toBe(termes.length);
  });

  it('n\'a pas cassé les entrées des autres modules (m1, m2, m4 toujours présentes)', () => {
    const ids = ['01-panorama-marches', '02-methodes-quantitatives', '04-taux-obligations'];
    for (const id of ids) {
      expect(glossaire.filter(e => e.moduleId === id).length, `module ${id}`).toBeGreaterThan(0);
    }
  });
});
