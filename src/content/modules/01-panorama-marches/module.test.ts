...
describe('module 1 — cibles de la spec (module culture, préférences utilisateur)', () => {
  const m = getModule('01-panorama-marches')!;

  it('est complet', () => {
    expect(m.chapitres.length).toBe(8); // 7 chapitres + synthèse
    expect(m.exercices.length).toBeGreaterThanOrEqual(7);
    expect(m.problemes.length).toBeGreaterThanOrEqual(10);
    expect(m.problemes.reduce((a, p) => a + p.scenarios.length, 0)).toBeGreaterThanOrEqual(30);
    expect(m.problemes.filter(p => p.difficulte === 4).length).toBeGreaterThanOrEqual(4);
    ...
    expect(m.formules.length).toBeGreaterThanOrEqual(8);
  });
