...
describe('flashcards module 4', () => {
  it('contient au moins 80 cartes', () => {
    expect(flashcards.length).toBeGreaterThanOrEqual(80);
  });

  it('a des ids uniques au format m4-fc-NN', () => {
    const ids = flashcards.map(c => c.id);
    expect(new Set(ids).size).toBe(ids.length);
    for (const id of ids) {
      expect(id).toMatch(/^m4-fc-\d{2}$/);
    }
  });
  ...
  it('respecte les quotas par tag : 30 définitions, 20 formules, 12 ordres de grandeur, 18 pièges', () => {
    ...
    expect(count('piege')).toBe(18);
  });

  it('chaque carte porte exactement un tag de type plus au moins un tag de thème', () => {
    const types = ['definition', 'formule', 'ordre-de-grandeur', 'piege'];
    for (const c of flashcards) {
