...
describe('flashcards module 2', () => {
  it('contient au moins 120 cartes', () => {
    expect(flashcards.length).toBeGreaterThanOrEqual(120);
  });

  it('a des ids uniques et bien formés couvrant m2-fc-001 à m2-fc-120', () => {
    const ids = flashcards.map(c => c.id);
    expect(new Set(ids).size).toBe(ids.length);
    for (const id of ids) {
      expect(id).toMatch(/^m2-fc-\d{3}$/);
    }
    const attendus = Array.from({ length: 120 }, (_, i) => `m2-fc-${String(i + 1).padStart(3, '0')}`);
  ...
  it('respecte les quotas par tag : 30 définitions, 20 formules, 10 ordres de grandeur, 24 pièges, 22 réflexes jury, 14 calculs mentaux', () => {
    ...
    expect(count('ordre-de-grandeur')).toBe(10);
...
describe('glossaire du module 2', () => {
  const entreesM2 = glossaire.filter(e => e.moduleId === M2);

  it('contient au moins 30 entrées pour le module 2', () => {
    expect(entreesM2.length).toBeGreaterThanOrEqual(30);
  });

  it('chaque entrée m2 a un terme, un terme anglais (en) et des définitions FR/EN non vides', () => {
    for (const e of entreesM2) {
  ...
  it('les termes m2 sont uniques', () => {
    const termes = entreesM2.map(e => e.terme);
